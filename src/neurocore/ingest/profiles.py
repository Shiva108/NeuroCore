"""Helpers for validating and resolving ingest profile defaults."""
...
def validate_ingest_profiles(
    ...
    for index, raw_profile in enumerate(raw_profiles):
        ...
        if "tags" in defaults:
            ...
            normalized_defaults["tags"] = list(dict.fromkeys(tag.strip() for tag in tags))
    ...
    *,
    ...
    for profile in raw_profiles:
        ...
        if not isinstance(match, dict):
            continue
        if not all(str(context.get(key, "")).strip() == value for key, value in match.items()):
            continue
